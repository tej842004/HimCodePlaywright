...
async function notes(page, visitKey) {
  try {
    ...
    // =========================================
    // 6. Wait for grid to settle
    // =========================================

    const rows = agGrid.locator('.ag-center-cols-container [role="row"]');

    // Give AG Grid time to populate if notes exist.
    ...
    // =========================================
    // IMPORTANT:
    // No Notes means return [].
    // Do NOT wait for rows.first().
    // =========================================

    if (rowCount === 0) {
      console.log('');
      console.log('No Notes found for this visit.');
      ...
      const result = await runPythonFunction('notes_data', [visitKey, emptyNotes]);

      if (result.success && result.result) {
        console.log(`Empty Notes data saved successfully for ${visitKey}.`);
      } else {
        console.log(`Empty Notes data was NOT saved for ${visitKey}.`);
      }
    ...
    if (result.success && result.result) {
      ...
      console.log(`Notes data was NOT saved for ${visitKey}.`);
    }

    // =========================================
    // 11. Return extracted data
    // =========================================
