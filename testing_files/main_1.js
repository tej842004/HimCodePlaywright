...
const { getUncodedVisits, markVisitAsDownloaded, closeDatabase } = require('../database/database');
...
async function downloadPhysicianOrders(page, visitNumber) {
  ...
  const downloadFolder = path.join(__dirname, 'him_coded_downloaded_files');
  ...
  for (let i = 0; i < count; i++) {
    console.log('\n----------------------------------------');
    console.log(`Processing Physician Orders ${i + 1}/${count}`);
    console.log('----------------------------------------');
    ...
    const baseFileName = `${visitNumber}_${i + 1}`;
...
async function processVisit(page, visit) {
  ...
  console.log(`SUCCESS: ${visit.visitKey} - downloads completed and database updated`);
}
